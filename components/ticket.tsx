...
const Ticket = forwardRef<HTMLDivElement, TicketProps>(
  (
    ...
    const { mutateAsync: doUpdateTicket, isPending: saveLoading } =
      useUpdateTicket(details.swimlaneId || 0);
    ...
    const [editableStartDate, setEditableStartDate] = useState(
      parseDate(details.startDate)
    );
    const [editableDueDate, setEditableDueDate] = useState(
      parseDate(details.dueDate)
    );
    const [editableSelectedTagIds, setEditableSelectedTagIds] = useState<
      Set<string>
    >(new Set(details.tagIds));
    const {
    ...
    const onDetailsSave = useCallback(async () => {
      try {
        await doUpdateTicket({
          ...
          tagIds: Array.from(editableSelectedTagIds).map((tagId) =>
            Number(tagId)
          ),
        });
      } catch (_) {
    ...
    const onMoveAction = useCallback(
      ...
      [details.id, doMoveTicket, onMoveClose]
    );
